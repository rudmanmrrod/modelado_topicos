...
var icon_fns = {"link" : function(ticks, i) {
    ...
    var id = ticks[0][0].baseURI.split("/").reverse()[3];
      base_fn(ticks,i)
        .attr("xlink:href","/static/img/link.png")
        .attr("class", "linkIcon icon")
        .on("click", function(d) { window.location.href = "/visualizacion/doc/" + id + "/" + k + "/" + encodeURIComponent(d);});
  },
  "ver" : function(ticks, i) {
    ...
    var id = ticks[0][0].baseURI.split("/").reverse()[3];
      base_fn(ticks,i)
        ...
        .on("click", function(d) { window.open("/visualizacion/see_doc/"+ id + "/" + k + "/"+d+"/",'_blank');});
  },
 "ap" : function(ticks, i) {
      base_fn(ticks,i)
